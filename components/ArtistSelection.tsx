      ...
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        {userArtists.length > 0 && (
          <span className="text-xs sm:text-sm text-gray-400 bg-gray-800 px-2 sm:px-3 py-1 rounded-full">
            {userArtists.length}/5 <span className="hidden xs:inline">selected</span>
          </span>
        )}
      </div>
    ...
    <div className="mb-4 sm:mb-6">
      <div className="relative">
        <input
          ...
          className="w-full bg-gray-800 text-white px-3 sm:px-4 py-3 sm:py-4 rounded-xl border border-gray-700 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20 text-sm sm:text-base"
        />
        {searching && (
...
function SearchResults({ searchResults, onAddArtist }: SearchResultsProps) {
  return (
    <div className="mt-3 bg-gray-800 rounded-xl border border-gray-700 max-h-32 sm:max-h-48 overflow-y-auto">
...
function SelectedArtistCard({ artist, onRemoveArtist }: SelectedArtistCardProps) {
  return (
    <div className="bg-gray-800 rounded-xl p-3 sm:p-4 text-center relative group border border-gray-700 hover:border-green-500 transition-colors">
      <button
        onClick={() => onRemoveArtist(artist.id)}
        className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 bg-red-500 hover:bg-red-400 text-white rounded-full w-5 h-5 sm:w-6 sm:h-6 text-xs sm:text-sm font-bold cursor-pointer transition-colors z-10"
      >
        ×
      </button>
      <img 
      ...
      <div className="relative group">
        ...
        {artist.name.length > 10 && (
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 sm:mb-2 px-2 py-1 bg-green-500 text-black text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 z-20 whitespace-nowrap pointer-events-none shadow-lg font-medium">
            {artist.name}
    ...
    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
      {userArtists.length > 0 && (
        <button
          ...
          className="w-full sm:flex-1 bg-gray-700 hover:bg-gray-600 text-white font-medium py-3 sm:py-4 px-4 sm:px-6 rounded-xl transition-colors text-sm sm:text-base"
        >
          Clear Artists
        </button>
      )}
      <button 
        ...
        className={`w-full ${userArtists.length > 0 ? 'sm:flex-1' : ''} bg-green-500 hover:bg-green-400 disabled:bg-gray-600 disabled:cursor-not-allowed text-black font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-xl transition-all duration-200 transform hover:scale-[1.02] disabled:hover:scale-100 text-sm sm:text-base`}
      >
        {generating ? (
          <div className="flex items-center justify-center gap-2 sm:gap-3">
            <div className="animate-spin rounded-full h-4 w-4 sm:h-5 sm:w-5 border-b-2 border-green-500"></div>
            <span className="text-xs sm:text-sm text-green-500">Generating...</span>
          </div>
        ) : (
          `Get Recommendations`
        )}
